 ...
 */
@SuppressLint("NewApi")
public class CiXiuColorAdapter extends Adapter<CiXiuColorAdapter.ViewHolder> {
	...
	public CiXiuColorAdapter(Context context) {
		super();
	...
	private void setContent(final ViewHolder viewHolder, final int position) {
		...
		if (mList != null && mList.get(position) != null) {
			x.image().bind(viewHolder.imageview1, mList.get(position).getPic(),
					mImageOptions);
			if ("1".equals(mList.get(position).getState())) {
				viewHolder.bg_rl
						.setBackgroundResource(R.drawable.select_color_bg);
			} else {
				viewHolder.bg_rl.setBackgroundColor(context.getResources()
