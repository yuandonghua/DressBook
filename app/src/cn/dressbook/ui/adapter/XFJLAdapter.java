 ...
 * @description: 消费记录适配器
 * @author:袁东华
 * @time:2015-11-9下午2:29:28
 */
public class XFJLAdapter extends Adapter<XFJLAdapter.ViewHolder> {
	private OnItemClickListener mOnItemClickListener;
	private OnItemLongClickListener mOnItemLongClickListener;
	private ArrayList<JYJL> mList;

	public XFJLAdapter(Context mContext, Handler mHandler) {
	...
	private void setContent(final ViewHolder viewHolder, final int position) {
		...
		if (mList != null && mList.size() > position) {
			...
			if (xfjl != null) {
				...
				if ("1".equals(xfjl.getDirection())) {
					viewHolder.num_tv.setText("+" + xfjl.getVary());
	...
	public ViewHolder onCreateViewHolder(ViewGroup parent, int arg1) {
		...
		View v = LayoutInflater.from(parent.getContext()).inflate(
				R.layout.xfjl_item, parent, false);
