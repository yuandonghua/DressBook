 ...
 */
public class DZSPDetailsParamAdapter extends
		Adapter<DZSPDetailsParamAdapter.ViewHolder> {
	private OnItemClickListener mOnItemClickListener;
	private OnItemLongClickListener mOnItemLongClickListener;
	private ArrayList<DZSPDetailsParams> mList;
	private Activity mContext;
	private ImageOptions mImageOptions = ManagerUtils.getInstance()
			.getImageOptions(ImageView.ScaleType.CENTER_CROP);
	private Handler mHandler;

	public DZSPDetailsParamAdapter(Activity mContext, Handler handler) {
		super();
	...
	public void setData(ArrayList<DZSPDetailsParams> mList) {
		this.mList = mList;
	}
	...
	private void setContent(final ViewHolder viewHolder, final int position) {
		...
		if (mList != null && mList.get(position) != null) {
			viewHolder.textView1.setText(mList.get(position).getparamName()
					+ ":" + mList.get(position).getparamValue());
		}
	}
	...
	public ViewHolder onCreateViewHolder(ViewGroup parent, int arg1) {
		...
		View v = LayoutInflater.from(parent.getContext()).inflate(
				R.layout.dzspdetailsparam_item, parent, false);
